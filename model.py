...
def apply_rotary_embeddings(x: torch.Tensor, m_theta_complex: torch.Tensor):
    ...
    # Ensure m_theta_complex is on the same device as x
    m_theta_complex = m_theta_complex.to(x.device)
...
class SelfAttention(nn.Module):
    ...
    def __init__(self, device, args: ModelArgs):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, m_theta_complex: torch.Tensor) -> torch.Tensor:
        ...
        xq = apply_rotary_embeddings(xq, m_theta_complex.to(self.device))
        xk = apply_rotary_embeddings(xk, m_theta_complex.to(self.device))
        ...
        # Ensure keys and values are moved to the correct device
        keys = keys.to(self.device)
        values = values.to(self.device)

        # Repeat the heads of K, V to match the number of heads of Q
        keys = repeat_kv(keys, self.n_rep).to(self.device)
        values = repeat_kv(values, self.n_rep).to(self.device)
        ...
        # Apply the mask: set scores of future positions to a large negative value
        ...
        return self.wo(output)  # Final linear transformation
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, device, args: ModelArgs):
        ...
        self.attention = SelfAttention(device, args)
...
class Transformer(nn.Module):
    def __init__(self, device, args: ModelArgs) -> None:
        super().__init__()
        ...
        for _ in range(args.n_layers):
            self.layers.append(EncoderBlock(device, self.args))
